...
FLAVORS_ROOT = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'flavors')
...
class Command(MakeMessagesCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--flavor",
            action="append",
            default=[settings.SHAREABOUTS.get('FLAVOR')],
            help="Flavor to generate messages for. Can be specified multiple times.",
        )
    ...
    def handle(self, *args, **options):
        flavors = options.pop('flavor', [])

        config_folders = [os.path.join(FLAVORS_ROOT, flavor) for flavor in flavors]
        mfile_paths = []

        for config_folder in config_folders:
            # Load the config file; don't preprocess translation strings
            print("Loading config file from:" + os.path.join(config_folder, 'config.yml'))
            config = get_shareabouts_config(config_folder, translate=False)

            # Generate an intermediary Python file
            mfile_path = os.path.relpath(os.path.join(config.path, '_config.translations.py'))
            print("Writing intermediary file " + mfile_path)
            with open(mfile_path, 'w') as mfile:
                mfile_paths.append(mfile_path)
        ...
        try:
            # Add the flavors to the settings' INSTALLED_APPS
            flavor_packages = [f'flavors.{flavor}' for flavor in flavors]
            settings.INSTALLED_APPS = tuple(flavor_packages + [
                package for package in settings.INSTALLED_APPS 
                if package not in flavor_packages
            ])
