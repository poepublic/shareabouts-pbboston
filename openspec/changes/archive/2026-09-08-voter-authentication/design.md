...
4. **Twilio SMS:** We will introduce the `twilio` Python package for interacting with Twilio to send SMS messages. To adhere to least-privilege security practices, authentication uses a Restricted API Key (`TWILIO_API_KEY`, `TWILIO_API_SECRET`) scoped by `TWILIO_ACCOUNT_SID` (strict cutover, no legacy auth token support).
